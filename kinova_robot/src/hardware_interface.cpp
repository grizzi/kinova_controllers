...
KinovaHardwareInterface::KinovaHardwareInterface(ros::NodeHandle& nh) : KortexArmDriver(nh)
{
  ...
  current_mode = KinovaControlMode::VELOCITY;
  mode = current_mode;
...
void KinovaHardwareInterface::update()
{
...
void KinovaHardwareInterface::write(const double dt)
{
  ...
  else if (mode_copy == KinovaControlMode::VELOCITY || mode_copy == KinovaControlMode::NO_MODE){}
  else{
...
bool KinovaHardwareInterface::send_command(){
  bool success = false;
  try{
...
ros::Time KinovaHardwareInterface::get_time()
{
  return ros::Time::now();
}
